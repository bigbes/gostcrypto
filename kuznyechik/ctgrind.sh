#!/bin/bash
# ctgrind.sh — EXPERIMENT. In-process constant-time check for NewCipherCT under
...
#   valgrind --trace-children=yes <bin> -test.fuzz=FuzzKuzCTLeak ...
set -uo pipefail

cd "$(dirname "$0")/.."
BIN=$(mktemp /tmp/ctg.XXXX.test)
CGO_ENABLED=1 go test -tags ctgrind -c -o "$BIN" ./kuznyechik/ || exit 1
...
echo "== ctgrind: Kuznyechik in-process constant-time check =="
...
# 2) Positive control: the table path MUST be flagged (test passes when
#    it detects the leak). A broken detector -> control fails -> we fail.
if ! valgrind --error-exitcode=0 --suppressions="$SUPP" \
     "$BIN" -test.run=TestKuzCTLeak_Control -test.count=1 >/dev/null 2>&1; then
  echo "FAIL: positive control did not detect the variable-time leak"; exit 1
fi
echo "[control] table Encrypt flagged — detector live"
